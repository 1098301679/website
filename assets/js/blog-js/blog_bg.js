...
let display_delay = 100;
let clearout_delay = 5000;
...
function startAnimation() {
   ...
   let interval = setInterval(function () {
      ...
      if (text[idx][ch] == ' ') {
         ...
         addLetter(div, text[idx][ch]);
      }
      // increase indices
      ch++;
      if (ch == text[idx].length) {
         idx++;
         ch = 0;
         div.append('<br>');
      }
...
function addLetter(div, item) {
   const id = guid();
   const elem = '<span class="raleway-dot"'
      + 'id="' + id + '">'
      + item + '</span>';
   div.append(elem);
   setTimeout(function () {
      div.find('#' + id).attr('class', 'raleway');
   }, clearout_delay);
}
...
function setClearoutDelay() {
   let length = 0;
   text.forEach(str => {
      length += str.length;
   });
   clearout_delay = length * display_delay;
}